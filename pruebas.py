...
print(W+W+W+h+u+u+u+u+u+t+t+t+s+s+s+s+r+r+r+q+q+q+p+p+o+o+o+n+n+n+m+m+m+l+l+l+l+k+k+k+j+j+j+j+j+i+W+W+W+W)
...
# Redondear los números en el segundo espacio de cada sublista
for sublista in lista_de_listas:
    sublista[1] = redondear_a_decena_mas_cercana(sublista[1])

# Calcular la suma total
suma_total = sum(sublista[1] for sublista in lista_de_listas)

# Si la suma total sobrepasa 360, ajustar los valores
while suma_total > 360:
    # Encontrar el valor más grande
    max_sublista = max(lista_de_listas, key=lambda x: x[1])
